...
int parse_files_with_policy(std::vector<std::string> &infile_list,
                            char *outfile, char *outfile_timestamps,
                            bool outfile_timestamps_sort_pts, int debug,
  ...
  for (const auto &infile : infile_list) {
    ...
    // --- Write CSV row ---
    fprintf(outfp,
            "%s,%i,%f,%f,%f,%s,%u,%u,%u,%i,%i,%i,%i,%i,%i,%i,%i,%f,%f,%f,%f,%i,"
            "%f,%f,%f,%u,%u,%f,%f,%f,%f,%i,%f,%f,%f,%f,%ld,%li,%d,%f,%s,%i,%d,%"
            "d,\"%s\",\"%s\"\n",
            ...
            num_video_frames, frame_rate_fps_median, frame_rate_fps_average,
            frame_rate_fps_reverse_average, frame_rate_fps_stddev,
            video_freeze ? 1 : 0, audio_video_ratio, duration_video_sec,
            duration_audio_sec, timescale_video_hz, timescale_audio_hz,
            pts_duration_sec_average, pts_duration_sec_median,
            pts_duration_sec_stddev, pts_duration_sec_mad, frame_drop_count,
            ...
            frame_drop_length_consecutive[0], frame_drop_length_consecutive[1],
            num_video_keyframes, key_frame_ratio, audio_type.c_str(),
            channel_count, sample_rate, sample_size, warn_str.c_str(),
            error_str.c_str());
...
int parse_files(std::vector<std::string> &infile_list, char *outfile,
  ...
  for (size_t i = 0; i < csv_keys.size(); ++i) {
    fprintf(outfp, "%s%s", csv_keys[i].c_str(),
            (i + 1 < csv_keys.size()) ? "," : "\n");
  }
